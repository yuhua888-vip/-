...
export function generateBigRoad(history) {
    if (!Array.isArray(history))
        throw new TypeError('Road history must be an array.');
    const columns = [];
    let leadingTies = 0;
    let lastWinner = null;
    let streakStart = -1;
    ...
    let row = 0;
    let tail = false;
    const occupied = (column, line) => columns[column]?.[line] != null;
    const put = (winner, tieCount) => {
        while (columns.length <= col)
            columns.push(Array(6).fill(null));
        const column = columns[col];
        if (column[row] != null)
            throw new Error('Road placement would overwrite an existing result.');
    ...
    for (const round of history) {
        ...
        if (lastWinner === null) {
            ...
            if (!tail && row < 5 && !occupied(col, row + 1))
                row += 1;
            else {
                tail = true;
                col += 1;
