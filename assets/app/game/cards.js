...
export function drawFront(ctx, card, width, height) {
    ctx.fillStyle = '#f4f0e7';
    ctx.fillRect(0, 0, width, height);
    const grain = ctx.createLinearGradient(0, 0, width, height);
    grain.addColorStop(0, '#ffffff45');
    grain.addColorStop(1, '#b5aaa51c');
    ctx.fillStyle = grain;
    ctx.fillRect(0, 0, width, height);
    ctx.strokeStyle = '#c9c6bd';
    ctx.lineWidth = Math.max(.5, width * .006);
    ctx.strokeRect(1, 1, width - 2, height - 2);
    ctx.fillStyle = isRed(card) ? '#962f43' : '#202c37';
    ...
    if (card.rank <= 10) {
        ...
        for (const position of pipRows[card.rank] ?? []) {
            const [x, y] = position;
            ctx.save();
            ctx.translate(width * (.15 + x * .7), height * (.08 + y * .84));
            if (y > .5)
                ctx.rotate(Math.PI);
            ctx.fillText(SUIT_SYMBOL[card.suit], 0, 0);
            ctx.restore();
        }
    }
    else {
        ctx.strokeStyle = isRed(card) ? '#962f4370' : '#344c5a70';
        ctx.lineWidth = width * .012;
        const crest = (flip) => { ctx.save(); if (flip) {
            ctx.translate(width, height);
            ctx.rotate(Math.PI);
        } ctx.beginPath(); ctx.moveTo(width * .31, height * .43); ctx.lineTo(width * .27, height * .29); ctx.lineTo(width * .41, height * .34); ctx.lineTo(width * .5, height * .23); ctx.lineTo(width * .59, height * .34); ctx.lineTo(width * .73, height * .29); ctx.lineTo(width * .69, height * .43); ctx.closePath(); ctx.stroke(); ctx.font = `${width * .28}px Georgia,serif`; ctx.fillText(rankLabel(card.rank), width * .5, height * .49); ctx.restore(); };
        crest(false);
        crest(true);
        ctx.font = `${width * .14}px Georgia,serif`;
        ctx.fillText(SUIT_SYMBOL[card.suit], width * .5, height * .5);
    }
}
export function drawBack(ctx, width, height) {
    ctx.fillStyle = '#f0eadf';
    ctx.fillRect(0, 0, width, height);
    ctx.fillStyle = '#183f47';
    ctx.fillRect(width * .045, height * .03, width * .91, height * .94);
    ctx.strokeStyle = '#9cbbb82e';
    ctx.lineWidth = .8;
    const step = width * .1;
    ...
    ctx.strokeStyle = '#b6cbc3';
    ctx.lineWidth = width * .008;
    ctx.strokeRect(width * .105, height * .075, width * .79, height * .85);
    ctx.fillStyle = '#17373f';
    ctx.save();
    ctx.translate(width * .5, height * .5);
    ctx.rotate(Math.PI / 4);
    ctx.fillRect(-width * .24, -width * .24, width * .48, width * .48);
    ctx.strokeRect(-width * .24, -width * .24, width * .48, width * .48);
    ctx.restore();
    ctx.fillStyle = '#d6ded0';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.font = `${width * .32}px Georgia,serif`;
    ctx.fillText('Q', width * .5, height * .49);
}
const cache = new Map();
export function cardImage(card) {
    const key = card ? `${card.suit}:${card.rank}` : 'back';
    ...
    if (card)
        drawFront(ctx, card, 240, 336);
    else
        drawBack(ctx, 240, 336);
...
export function cardElement(card) {
    ...
    image.alt = card ? `${SUIT_SYMBOL[card.suit]} ${rankLabel(card.rank)}` : '待开牌';
    return image;
}
