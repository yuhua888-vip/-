...
export function deformPaper(x, y, progress, corner, direction = .5) {
...
export function resistedProgress(raw, resistance) {
    const p = Math.max(0, Math.min(1, raw));
    return p / (resistance + (1 - resistance) * p);
}
export function springStep(position, velocity, target, seconds, stiffness) {
...
function texture(card) {
    const canvas = document.createElement('canvas');
    canvas.width = 300;
    canvas.height = 420;
    const ctx = canvas.getContext('2d');
    if (!ctx)
        throw new Error('Card texture unavailable');
    if (card)
        drawFront(ctx, card, 300, 420);
    else
        drawBack(ctx, 300, 420);
    return canvas;
}
...
export class PaperRenderer {
    ...
    back;
    count = 0;
    last = { progress: 0, corner: 'br', direction: .5 };
    mode = 'canvas';
    constructor(host) {
        this.host = host;
        ...
        try {
            ...
            console.warn('Paper GL unavailable; using Canvas', error);
            this.fallback();
        }
        this.resize();
    }
    uniform(name) { return this.uniforms.get(name) ?? null; }
    setupGL() {
        ...
        this.gl = gl;
        const compile = (type, code) => { const shader = gl.createShader(type); gl.shaderSource(shader, code); gl.compileShader(shader); if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        ...
        for (const name of ['u_progress', 'u_corner', 'u_direction', 'u_shadow', 'u_front', 'u_back'])
            this.uniforms.set(name, gl.getUniformLocation(program, name));
        const cols = 36, rows = 50, vertices = [], indices = [];
        for (let y = 0; y <= rows; y++)
            for (let x = 0; x <= cols; x++)
                vertices.push(x / cols, y / rows);
        for (let y = 0; y < rows; y++)
            for (let x = 0; x < cols; x++) {
        ...
        gl.uniform1i(this.uniform('u_front'), 0);
    ...
    lost = (event) => { event.preventDefault(); this.fallback(); this.resize(); this.render(this.last.progress, this.last.corner, this.last.direction); };
    fallback() {
        this.release();
        const replacement = document.createElement('canvas');
        replacement.id = 'peek-canvas';
    ...
    resize() {
        ...
        this.canvas.width = Math.round(PAPER.viewportWidth * dpr);
        this.canvas.height = Math.round(PAPER.viewportHeight * dpr);
        this.gl?.viewport(0, 0, this.canvas.width, this.canvas.height);
        this.ctx?.setTransform(dpr, 0, 0, dpr, 0, 0);
    }
    setCard(card) {
        this.front = texture(card);
        const gl = this.gl;
        if (gl)
            for (const [unit, source] of [this.front, this.back].entries()) {
                gl.activeTexture(gl.TEXTURE0 + unit);
                gl.bindTexture(gl.TEXTURE_2D, this.textures[unit]);
                gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, source);
            }
    }
    render(progress, corner, direction = .5) {
        this.last = { progress, corner, direction };
        this.host.dataset.renderer = this.mode;
        const gl = this.gl;
        if (gl) {
            gl.clearColor(0, 0, 0, 0);
            gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
            gl.uniform1f(this.uniform('u_progress'), progress);
            gl.uniform2f(this.uniform('u_corner'), corner.endsWith('r') ? 1 : 0, corner.startsWith('b') ? 1 : 0);
            gl.uniform1f(this.uniform('u_direction'), direction);
            gl.uniform1f(this.uniform('u_shadow'), 1);
            gl.depthMask(false);
            gl.drawElements(gl.TRIANGLES, this.count, gl.UNSIGNED_SHORT, 0);
            gl.depthMask(true);
            gl.uniform1f(this.uniform('u_shadow'), 0);
            gl.drawElements(gl.TRIANGLES, this.count, gl.UNSIGNED_SHORT, 0);
        }
        else
            this.drawCanvas(progress, corner, direction);
    }
    drawCanvas(progress, corner, direction) {
        const ctx = this.ctx;
        if (!ctx || !this.front)
            return;
        ctx.clearRect(0, 0, 420, 550);
        if (progress < .001 || progress > .999) {
            ctx.drawImage(progress > .999 ? this.front : this.back, 60, 65, 300, 420);
            return;
        }
        const cols = 12, rows = 18, vertices = [];
        for (let y = 0; y <= rows; y++)
            for (let x = 0; x <= cols; x++) {
                const p = deformPaper(x / cols * 300, y / rows * 420, progress, corner, direction), perspective = 1 - p.z / 2400;
                vertices.push({ ...p, x: (p.x - 150) / perspective + 210, y: (p.y - 210) / perspective + 275, u: x / cols * 300, v: y / rows * 420 });
            }
        const triangles = [];
        for (let y = 0; y < rows; y++)
            for (let x = 0; x < cols; x++) {
        ...
        for (const ids of triangles) {
            let [a, b, c] = ids.map(i => vertices[i]);
            ...
            const det = a.u * (b.v - c.v) + b.u * (c.v - a.v) + c.u * (a.v - b.v);
            if (Math.abs(det) < .001)
                continue;
            const matrix = (key) => [(a[key] * (b.v - c.v) + b[key] * (c.v - a.v) + c[key] * (a.v - b.v)) / det, (a[key] * (c.u - b.u) + b[key] * (a.u - c.u) + c[key] * (b.u - a.u)) / det, (a[key] * (b.u * c.v - c.u * b.v) + b[key] * (c.u * a.v - a.u * c.v) + c[key] * (a.u * b.v - b.u * a.v)) / det];
            const mx = matrix('x'), my = matrix('y');
            ctx.save();
            ctx.beginPath();
            ctx.moveTo(a.x, a.y);
            ctx.lineTo(b.x, b.y);
            ...
            ctx.closePath();
            ctx.clip();
            ctx.transform(mx[0], my[0], mx[1], my[1], mx[2], my[2]);
            ctx.drawImage(front ? this.front : this.back, 0, 0);
            ctx.fillStyle = `rgba(0,0,0,${(1 - Math.abs((a.normal + b.normal + c.normal) / 3)) * .22})`;
            ctx.fillRect(0, 0, 300, 420);
            ctx.restore();
        }
    }
    release() { const gl = this.gl; if (gl) {
    ...
    } this.canvas.removeEventListener('webglcontextlost', this.lost); this.gl = null; this.program = null; this.buffers = []; this.textures = []; this.uniforms.clear(); }
    dispose() { this.release(); this.front = null; }
}
