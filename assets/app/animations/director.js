...
export const timelineObservers = new Set();
...
export class AnimationDirector {
    ...
    async play(name, cues) {
        for (const cue of cues) {
            ...
            try {
                cue.enter?.();
                await (cue.run ? cue.run(cue.duration) : this.motion.wait(cue.duration));
                this.motion.signal.throwIfAborted();
                emit('end');
            }
            catch (error) {
...
export function dealTiming(deal) {
...
export function revealTiming(deal, visible) {
