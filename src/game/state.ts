export const PHASES = ['BETTING_OPEN', 'BETTING_WARNING', 'BETTING_CLOSED', 'PREPARE_DEAL', 'DEAL_PLAYER', 'DEAL_BANKER', 'PLAYER_PEEK', 'BANKER_PEEK', 'PLAYER_DRAW', 'BANKER_DRAW', 'REVEAL', 'RESULT', 'COLLECT_CHIPS', 'PAYOUT', 'RESET'] as const;
...
export class StateMachine {
  ...
  move(next: Phase): void {
    if (!transitions[this.phase].includes(next)) throw new Error(`非法牌局状态：${this.phase} → ${next}`);
    this.phase = next; this.onChange(next);
  }
  recoverToBetting(): void { this.phase = 'BETTING_OPEN'; this.onChange(this.phase); }
  get canBet(): boolean { return this.phase === 'BETTING_OPEN'; }
}
